...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "shuv2code-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-shuv2code", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "shuv2code", specifier: pluginPath }],
