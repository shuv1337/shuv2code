...
Load this reference only when inspecting or seeding local shuv2code state directly.
...
```bash
node apps/server/scripts/shuv2code-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/shuv2code-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/shuv2code-seed.sql
```
