---
name: test-shuv2code-mobile
description: Launch and test shuv2code Mobile on an iOS Simulator or Android Emulator against disposable local shuv2code environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test shuv2code Mobile

Run one focused, end-to-end mobile verification pass against disposable shuv2code state. Use the sibling [`test-shuv2code-app`](../test-shuv2code-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `shuv2code Dev`
- Bundle/package identifier: `dev.shuv.shuv2code.dev`
- URL scheme: `shuv2code-dev`
...
## Start one disposable shuv2code environment

Run backend commands from the repository root. Use the ignored, worktree-local `.shuv2code` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.shuv2code` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-shuv2code-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `shuv2code-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/Shuv2CodeDev.xcworkspace`
- Scheme: `Shuv2CodeDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `dev.shuv.shuv2code.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> dev.shuv.shuv2code.dev app
...
```bash
adb -s <emulator-serial> shell pm path dev.shuv.shuv2code.dev
...
In PowerShell, set `$env:SHUV2CODE_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'shuv2code-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'shuv2code-dev://connections/new' \
  dev.shuv.shuv2code.dev
```

Run only the command for the selected platform.

In shuv2code Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in shuv2code when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from shuv2code Dev.
