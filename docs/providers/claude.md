...
This guide is for people who want to use more than one Claude setup in shuv2code.
...
An empty `Claude HOME path` means shuv2code uses your normal home directory.
...
shuv2code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so shuv2code keeps separate Claude homes isolated instead of
...
2. Add or configure a Claude provider in shuv2code.
