...
The launcher is the only runtime writer of `service-state.json`. `shuv2code service install` and
`shuv2code service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `shuv2code@<target>` into a unique staging directory.
...
```sh
npx shuv2code@<version> service update
```
