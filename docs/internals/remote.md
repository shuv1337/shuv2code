...
shuv2code has one runtime boundary: a client talks to a shuv2code server over HTTP and WebSocket, and the server
...
One running shuv2code server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed shuv2code connect relay tunnels.                                 |
...
Tailscale is the first provider, and shuv2code manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@shuv2code/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a shuv2code server? It does not answer
how the server got started or who manages the process.

### Direct WebSocket access

`wss://shuv2code.example.com` or `ws://10.0.0.15:3773`, paired as a bearer target. This is the base model.
...
Managed shuv2code connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
Worker itself. See [shuv2code-connect.md](./shuv2code-connect.md).

### Tailscale access

A shuv2code-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote shuv2code
...
Launch answers a different question: how does a shuv2code server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs shuv2code and the client connects directly or through a
  tunnel.
...
- **Client-managed local publish.** A local server is published through the relay with
  `shuv2code connect link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
...
- a relay-hosted OAuth callback broker (see [shuv2code-connect.md](./shuv2code-connect.md));
