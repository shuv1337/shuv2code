...
The screenshot harness runs the real mobile application against three disposable local shuv2code
...
the software keyboard, and supplies deterministic shuv2code connect discovery rows to the real
...
1. Creates three temporary shuv2code base directories and starts a local server for each on an available
   port.
2. Creates shuv2code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic shuv2code review diff.
...
`pnpm --filter @shuv2code/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real shuv2code and React fixture
