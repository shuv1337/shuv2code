...
- `.github/workflows/release.yml` is the sole manual npm publishing workflow.
  ...
  verification once, builds one npm tarball, clean-installs both CLI names, and
  uploads the tarball and desktop artifacts with SHA-256 files. Publish mode is
  `main`-only, requires iOS Sqim confirmation and approval through the protected
  `production` environment, verifies the downloaded tarball digest, publishes it
  directly with npm OIDC and the SemVer-derived dist-tag (`latest` / `next` /
  `nightly`), proves the exact npm version is observable, and only then creates
  the GitHub release. Release jobs do not use dependency caches or npm write
