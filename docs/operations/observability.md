...
shuv2code has one server-side observability model:
...
stdout/stderr at `~/.shuv2code/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.shuv2code/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.shuv2code/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.shuv2code/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.shuv2code/dev/logs/server.trace.ndjson`.
...
```bash
export SHUV2CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export SHUV2CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export SHUV2CODE_OTLP_SERVICE_NAME=shuv2code-local
```

Optional:

```bash
export SHUV2CODE_TRACE_MIN_LEVEL=Info
export SHUV2CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `SHUV2CODE_OTLP_*`.

macOS app bundle example:

```bash
SHUV2CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
SHUV2CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
SHUV2CODE_OTLP_SERVICE_NAME=shuv2code-desktop \
"/Applications/shuv2code.app/Contents/MacOS/shuv2code"
```
...
```bash
TRACE_FILE="${SHUV2CODE_HOME:-$HOME/.shuv2code}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.shuv2code/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.shuv2code/dev/logs/server.trace.ndjson"
```
...
- `SHUV2CODE_OTLP_TRACES_URL` was not set
...
- `SHUV2CODE_TRACE_FILE`: override trace file path
- `SHUV2CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `SHUV2CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `SHUV2CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `SHUV2CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `SHUV2CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`
