...
- Reads the shared production shuv2code connect relay URL and Clerk client configuration before packaging clients.
...
## shuv2code connect relay deployment
...
```sh
vp run --filter shuv2code-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `SHUV2CODE_WEB_ROUTER_URL`: defaults to `https://app.shuv.me`.
- `SHUV2CODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.shuv.me`.
- `SHUV2CODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.shuv.me`.

Required Vercel domains:

- `app.shuv.me`: the router domain users open, updated by stable releases.
- `latest.app.shuv.me`: channel alias updated by stable releases.
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.shuv.me` points at a deployment containing the router
...
- Repository slug source:
  - `SHUV2CODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
