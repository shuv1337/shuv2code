...
shuv2code can update a connected server to the exact version of the client that detected version
...
| ----------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `boot-service`    | Linux server running under the shuv2code-managed systemd user service                         | Call the update RPC; the service unit is replaced and restarted.      |
...
flowchart TD
    ...
    E --> F[Install exact shuv2code version in pinned runtime]
    F --> G[Run version preflight]
    G -->|fails| H[Remove failed runtime and keep current server]
    G -->|passes| I{Handoff method}
    I -->|boot-service| J[Rewrite and restart shuv2code systemd unit]
...
The update service permits one update at a time. It installs `shuv2code@<version>` under
...
The systemd user service is a host lifecycle concern, not a shuv2code connect resource. The standalone
`shuv2code service install`, `uninstall`, `update`, and `status` commands own it. Install and update both
reconcile the unit through `BootService`; running `npx shuv2code@latest service update` therefore pins and
activates the latest CLI release without requiring a connected client.

The `shuv2code connect` onboarding flow may offer service installation, but it calls the same reconciliation
operation as `shuv2code service install`. Connect logout only disables cloud access and clears its
authorization; it does not uninstall the host service.

## Process Handoff

For `boot-service`, the server atomically rewrites the shuv2code-managed user unit to point at the verified
...
The exact client version must exist as the `shuv2code` npm package before a client carrying that version is
