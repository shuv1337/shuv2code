...
This document describes the target architecture for first-class remote environments in shuv2code.
...
- Keep the shuv2code server as the execution boundary.
...
shuv2code already has a clean runtime boundary: the client talks to a shuv2code server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one shuv2code server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one shuv2code server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the shuv2code runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running shuv2code server instance.
...
How does the client speak WebSocket to a shuv2code server?
...
- best fit for hosted or self-managed remote shuv2code deployments
...
For shuv2code, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a shuv2code-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote shuv2code server
...
How does a shuv2code server come to exist on the target machine?
...
The user or operator already runs shuv2code on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in shuv2code:
...
The recommended shuv2code flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies shuv2code availability.
3. Desktop launches or reuses a remote shuv2code server.
...
This is the inverse of remote launch: a local shuv2code server is already running, and the client publishes it through a tunnel.
...
- A manually hosted shuv2code server might be reached through direct `wss`.
...
shuv2code already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
shuv2code should not copy that part.

shuv2code already has the right runtime boundary:

- one shuv2code server per environment
- ordinary HTTP/WebSocket between client and environment

So shuv2code should borrow Zed's launch discipline, not its transport protocol.
