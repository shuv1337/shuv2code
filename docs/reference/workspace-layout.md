...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `shuv2code` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@shuv2code/shared/git`, `@shuv2code/shared/DrainableWorker`) — no barrel index.
