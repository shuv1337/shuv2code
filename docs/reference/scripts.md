...
- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.shuv2code`, even when `SHUV2CODE_HOME` is set, storing state in `<worktree>/.shuv2code/userdata`. Pass `--home-dir <path>` to choose another isolated directory explicitly. Submodules are not worktrees and keep the normal precedence.
- From the **main checkout**, dev commands implicitly use `~/.shuv2code/dev`, keeping development state separate from `~/.shuv2code/userdata`. An explicit `--home-dir <path>` stores state under `<path>/userdata`; the base directory remains available for caches, worktrees, and other shared data.
- Web dev commands do not auto-open a browser. Open the one-time pairing URL printed by the server so the first browser navigation is authenticated. Set `SHUV2CODE_NO_BROWSER=0` only when interactive auto-open is intentional.
- Pass dev-runner flags directly after the root task name, for example:
  `vp run dev --home-dir /tmp/shuv2code-dev`
...
- `node apps/server/scripts/shuv2code-sqlite-state.ts <query|exec> --base-dir <path> ...` — Inspects or seeds an isolated shuv2code SQLite database; writes create a private backup first.
...
- Desktop production windows load the bundled UI from `shuv2code://app/index.html` (not a `127.0.0.1` document URL).

- Desktop packaging includes `apps/server/dist` (the `shuv2code` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
- Signed macOS builds also require `SHUV2CODE_APPLE_TEAM_ID` and
  `SHUV2CODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `SHUV2CODE_CLERK_PUBLISHABLE_KEY` unless `SHUV2CODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
Set `SHUV2CODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `13773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `SHUV2CODE_DEV_INSTANCE`)
- Example: `SHUV2CODE_DEV_INSTANCE=branch-a vp run dev:desktop`

If you want full control instead of hashing, set `SHUV2CODE_PORT_OFFSET` to a numeric offset.
