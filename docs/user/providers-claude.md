...
This guide is for people who want to use more than one Claude setup in shuv2code. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install shuv2code](./install.md).
...
In shuv2code Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means shuv2code uses Claude Code's normal config directory.

When you set this field, shuv2code points Claude Code at that directory with the
...
shuv2code looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where shuv2code looks.

Then add another Claude provider in shuv2code:
...
shuv2code only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so shuv2code keeps separate config directories isolated
...
Add or edit a Claude provider in shuv2code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. shuv2code stores the value as a server secret and does not
...
shuv2code does not need a special Claude Code Router provider. Treat the router as a Claude
