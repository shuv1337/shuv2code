...
shuv2code connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

shuv2code works with the platforms your team already uses:
...
- shuv2code can suggest titles and descriptions based on your commits
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in shuv2code
...
1. Install the GitHub CLI on the machine running shuv2code:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in shuv2code and verify GitHub shows as authenticated
...
machine running shuv2code.

Recommended, a Bitbucket access token:

```bash
export SHUV2CODE_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
```bash
export SHUV2CODE_BITBUCKET_EMAIL="you@example.com"
export SHUV2CODE_BITBUCKET_API_TOKEN="your-token"
```

If both are set, the access token wins. Restart shuv2code and verify the connection in **Source
Control settings**.
...
**Git is required** – shuv2code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running shuv2code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
