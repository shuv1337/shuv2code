...
> =>
  Effect.gen(function* () {
    ...
    const tempDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "shuv2code-wsl-server-tree-test-",
    });
    ...
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        const marker = yield* fileSystem.readFileString(
          path.join(root, "shuv2code-wsl-server-tree.json"),
        );
  ...
  it.effect("sweeps stale version directories and leftover partials after extraction", () =>
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        // SHUV2CODE_HOME is set to tempDir, so the desktop state dir resolves to
        // <tempDir>/userdata (no .shuv2code segment).
