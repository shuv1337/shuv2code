...
import { reportAtomCommandResult, settlePromise } from "@shuv2code/client-runtime/state/runtime";
...
/**
 * Post-sign-in onboarding sheet for shuv2code connect. Mobile never publishes
 * environments itself — it consumes ones published elsewhere — so this simply
 * surfaces the account's shuv2code connect environments right after sign-in so every
...
function ConfiguredConnectOnboardingRouteScreen() {
  ...
  return (
    <View collapsable={false} className="flex-1 bg-sheet">
      {Platform.OS === "android" ? (
        <AndroidSheetHeader
          title="Set up shuv2code connect"
        ...
        {isSignedIn ? (
          ...
          <View collapsable={false} className="rounded-[24px] bg-card p-5">
            <Text className="text-sm leading-normal text-foreground-muted">
              Sign in to your shuv2code account to set up shuv2code connect.
            </Text>
          </View>
        )}
