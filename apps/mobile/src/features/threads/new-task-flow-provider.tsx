...
} from "@shuv2code/shared/threadEnvMode";
...
import { EnvironmentProject } from "@shuv2code/client-runtime/state/shell";
import { type VcsRef } from "@shuv2code/client-runtime/state/vcs";
import {
...
export function NewTaskFlowProvider(props: React.PropsWithChildren) {
  ...
  // checked-in shuv2code.json, then the server's configured default.
  const shuv2CodeProjectFileQuery = useEnvironmentQuery(
    selectedProject !== null && selectedProject.workspaceRoot !== ""
      ? projectEnvironment.readFile({
          environmentId: selectedProject.environmentId,
          input: { cwd: selectedProject.workspaceRoot, relativePath: SHUV2CODE_PROJECT_FILE_NAME },
        })
      : null,
  );
  const shuv2CodeProjectFileData = shuv2CodeProjectFileQuery.data as ProjectReadFileResult | null;
  const shuv2CodeProjectFileDefaultMode = useMemo(() => {
    if (shuv2CodeProjectFileData === null || shuv2CodeProjectFileData.truncated) return null;
    return (
      parseShuv2CodeProjectFile(shuv2CodeProjectFileData.contents)?.defaultThreadEnvMode ?? null
    );
  }, [shuv2CodeProjectFileData]);
  const defaultWorkspaceMode: WorkspaceMode = resolveDefaultThreadEnvMode({
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFile: shuv2CodeProjectFileDefaultMode,
  ...
  // the frozen interim value beats the shuv2code.json default once it loads.
  const defaultWorkspaceModeSettled = isDefaultThreadEnvModeSettled({
    explicitMode: selectedProjectDraft.workspaceSelection?.mode,
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFilePending: shuv2CodeProjectFileQuery.isPending,
  });
