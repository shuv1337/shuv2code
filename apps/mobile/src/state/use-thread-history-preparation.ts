...
function confirmMigration(threadTitle: string): Promise<boolean> {
  const title = `Update “${threadTitle}” to the current thread format?`;
  const message =
    "This legacy Codex thread needs a one-time migration before Shuv can load it efficiently. Codex will rewrite only this thread's persisted rollout as paginated history. No other threads will be changed.";
  ...
  useEffect(() => {
    ...
    void inspectOnce(input.environmentId, input.threadId)
      .then(async (inspected) => {
        ...
        const confirmed = await confirmMigration(input.threadTitle);
        if (!active) return;
        if (!confirmed) {
