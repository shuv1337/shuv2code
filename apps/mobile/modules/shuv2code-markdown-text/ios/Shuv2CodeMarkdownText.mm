...
#import "Shuv2CodeMarkdownTextShadowNode.h"
#import "Shuv2CodeMarkdownTextComponentDescriptor.h"
...
static void Shuv2CodeMarkdownTextApplyParagraphStyles(
...
static void Shuv2CodeMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@protocol Shuv2CodeMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface Shuv2CodeMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<Shuv2CodeMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<Shuv2CodeMarkdownOutsideTapTarget>)target;

@end

static const void *Shuv2CodeMarkdownOutsideTapCoordinatorKey =
    &Shuv2CodeMarkdownOutsideTapCoordinatorKey;

@implementation Shuv2CodeMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<Shuv2CodeMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<Shuv2CodeMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, Shuv2CodeMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<Shuv2CodeMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static Shuv2CodeMarkdownOutsideTapCoordinator *
Shuv2CodeMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  Shuv2CodeMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, Shuv2CodeMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[Shuv2CodeMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        Shuv2CodeMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface Shuv2CodeMarkdownText () <RCTShuv2CodeMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface Shuv2CodeMarkdownText () <Shuv2CodeMarkdownOutsideTapTarget>
@end

@implementation Shuv2CodeMarkdownText {
  UIView * _view;
  UITextView * _textView;
  Shuv2CodeMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<Shuv2CodeMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const Shuv2CodeMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    Shuv2CodeMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, Shuv2CodeMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [Shuv2CodeMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<Shuv2CodeMarkdownTextProps const>(_props);
  ...
  Shuv2CodeMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  Shuv2CodeMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::Shuv2CodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::Shuv2CodeMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<Shuv2CodeMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<Shuv2CodeMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<Shuv2CodeMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == Shuv2CodeMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == Shuv2CodeMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == Shuv2CodeMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == Shuv2CodeMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const Shuv2CodeMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (Shuv2CodeMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[Shuv2CodeMarkdownTextRun class]]) {
      continue;
    }

    Shuv2CodeMarkdownTextRun* textChild = (Shuv2CodeMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::Shuv2CodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::Shuv2CodeMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> Shuv2CodeMarkdownTextCls(void)
{
  return Shuv2CodeMarkdownText.class;
}
