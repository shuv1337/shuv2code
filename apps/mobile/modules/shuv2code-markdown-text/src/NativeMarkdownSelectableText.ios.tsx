...
export function NativeMarkdownSelectableText(props: {
  ...
  // Shuv2CodeMarkdownText only rebuilds its attributed string during native layout. A
      ...
      {keyedRuns.map(({ key, run, text }) => {
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              run.fileIcon
                ? `shuv2code-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                : run.skillName
                  ? "shuv2code-skill:sf:cube"
                  : undefined
            }
