...
public final class Shuv2CodeKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("Shuv2CodeKeyboardCommands")

    View(Shuv2CodeKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: Shuv2CodeKeyboardCommandsView, commands: [String]) in
...
public final class Shuv2CodeKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.shuv2codeFirstResponder == nil else { return }
...
private extension UIView {
  var shuv2codeFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.shuv2codeFirstResponder { return responder }
    }
    return nil
  }
}
