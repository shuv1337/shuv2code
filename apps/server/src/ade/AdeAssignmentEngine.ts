...
>()("shuv2code/ade/AdeAssignmentKernelPort") {
   ...
   * unwired port must not block every admission), but nothing can be
   * delivered or steered, and no session is adoptable — so recovery takes the
   * conservative `needs-resume` path rather than a silent restart.
   */
  static readonly layerUnwired = Layer.succeed(AdeAssignmentKernelPort, {
...
export interface StartAssignmentOutcome {
  readonly assignment: Assignment;
  /**
   * True when admission found the recipient's kernel down and parked the
   * assignment as `blocked: kernel-down` instead of running it.
   */
...
export interface AdeAssignmentEngineShape {
  ...
  /** Admit queued/blocked work into `running`, or park it on a down kernel. */
    ...
    Effect.gen(function* () {
      ...
      const createAssignment: AdeAssignmentEngineShape["createAssignment"] = Effect.fn(
        ...
        return yield* sql
          .withTransaction(
            Effect.gen(function* () {
              ...
              if (inserted.length === 0) {
                // Idempotent replay (ADR §13.6): the unique index on
                // (requester_kind, COALESCE(requester_bot_id, ''), key) already
                // holds this request. Durable, so it survives restarts — which
                // is exactly what the gate's in-memory dedupe cannot do.
                const existing = yield* sql<AssignmentRow>`
          ...
          .pipe(
            Effect.catchTag("SqlError", (cause) => Effect.fail(mapSql("createAssignment")(cause))),
          );
      });
      ...
      const reorderQueue: AdeAssignmentEngineShape["reorderQueue"] = Effect.fn(
        ...
        return yield* sql
          ...
          .pipe(Effect.catchTag("SqlError", (cause) => Effect.fail(mapSql("reorderQueue")(cause))));
      });
      ...
      const activePrimaryBinding = Effect.fn("AdeAssignmentEngine.activePrimaryBinding")(function* (
        botId: BotId,
      ) {
      ...
      const startAssignment: AdeAssignmentEngineShape["startAssignment"] = Effect.fn(
        ...
        // Admission-time kernel check. The S17 checker is edge-triggered, so
        // an assignment entering `running` mid-outage would otherwise never be
        // flipped: park it here instead (ADR §11.3 queue-and-alert).
        if (binding !== null) {
          const health = yield* port.kernelHealth(binding.engine);
          if (!health.available) {
      ...
      const cancelAssignment: AdeAssignmentEngineShape["cancelAssignment"] = Effect.fn(
        ...
        return yield* sql
          .withTransaction(
            Effect.gen(function* () {
              ...
              if (input.cascade) {
                ...
                // (§13.4). Breadth-first over parent_assignment_id.
          ...
          .pipe(
            Effect.catchTag("SqlError", (cause) => Effect.fail(mapSql("cancelAssignment")(cause))),
          );
      });
      ...
      const reportResult: AdeAssignmentEngineShape["reportResult"] = Effect.fn(
        ...
        return yield* sql
          ...
          .pipe(Effect.catchTag("SqlError", (cause) => Effect.fail(mapSql("reportResult")(cause))));
      });
      ...
      /**
       * Send one claimed batch and record the outcome. Success marks the rows
       * `delivered` (the product-level delivery record) and releases a settled
       * parental wait; failure returns them to `pending` for the next drain.
       */
      const sendClaimedBatch = Effect.fn("AdeAssignmentEngine.sendClaimedBatch")(function* (
        ...
        if (sent._tag === "Failure") {
          yield* Effect.logWarning("ADE assignment delivery failed; batch returned to pending", {
          ...
          yield* sql`
            UPDATE ade_assignments
            SET delivery_state = 'pending', delivery_attempt_id = NULL, updated_at = ${at}
          ...
          .withTransaction(
            Effect.gen(function* () {
              ...
              if (batch.parentAssignmentId !== null) {
                // The wait is settled: every child of this parent is terminal
                // and its results have now landed in one message.
                yield* sql`
                  UPDATE ade_assignments
        ...
        for (const [key, group] of groups) {
          ...
          for (const row of group.rows) {
            ...
            const taken = yield* sql<{ assignment_id: string }>`
              UPDATE ade_assignments
              SET delivery_state = 'delivering', delivery_attempt_id = ${deliveryKey}, updated_at = ${at}
        ...
        Effect.fn("AdeAssignmentEngine.recoverInterruptedDeliveries")(function* () {
          ...
          for (const [deliveryKey, group] of byAttempt) {
            ...
            const parentAssignmentId =
              group.every((row) => row.parent_assignment_id === first.parent_assignment_id) &&
              first.parent_assignment_id !== null
                ? (first.parent_assignment_id as AssignmentId)
                : null;
