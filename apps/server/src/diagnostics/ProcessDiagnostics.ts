...
>()("shuv2code/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the shuv2code server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signaled: false,
          message: Option.some(
            `Process ${input.pid} is not a signalable shuv2code backend descendant.`,
          ),
        };
      }
