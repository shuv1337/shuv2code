        ...
        Effect.gen(function* () {
          ...
          const bootstrapProgram = Effect.gen(function* () {
            ...
            if (bootstrap?.prepareWorktree) {
              ...
              const activeVcs = bootstrap.prepareWorktree.startFromOrigin
                ? yield* vcsDriverRegistry.resolve({
                    cwd: bootstrap.prepareWorktree.projectCwd,
                  })
                : null;
              // "Start from origin" is a stored default; Git repos without an
              // origin remote fall back to the local base branch instead of
              // failing the whole bootstrap on `git fetch origin`. The
              // remote-exists pre-check is Git-only; jj keeps the fork's
              // VCS-agnostic fetch path.
              ...
              if (startFromOrigin && activeVcs !== null) {
