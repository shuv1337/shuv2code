...
export interface TelemetryCounters {
  ...
  readonly allShuv2Code: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allShuv2Code: ResourceTelemetryAggregate;
  };
...
export const emptyTelemetryCounters = (): TelemetryCounters => ({
  ...
  allShuv2Code: emptyGroupCounters(),
});
...
function applyLifecycleCounters(input: {
  ...
  let allShuv2Code = input.counters.allShuv2Code;
  for (const processDelta of input.deltas) {
    ...
    allShuv2Code = incrementCounters(allShuv2Code, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allShuv2Code = incrementCounters(allShuv2Code, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allShuv2Code = incrementCounters(allShuv2Code, { processExits: 1 });
  }

  return { backend, electron, monitor, allShuv2Code };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allShuv2Code: aggregate(ordered, counters.allShuv2Code),
    },
    deltas: processDeltas,
  };
}
