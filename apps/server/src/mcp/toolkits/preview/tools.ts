...
export const PreviewNavigateTool = safeBrowserTool(
  Tool.make("preview_navigate", {
    description:
      "Navigate a collaborative browser tab. Pass tabId to target a specific tab, plus {url:'https://shuv2code.chat'} for a website or {target:{kind:'environment-port',port:5173}} for a dev server. Exactly one of url or target is required.",
...
export const PreviewSnapshotTool = readonlyBrowserTool(
  Tool.make("preview_snapshot", {
    description:
      "Inspect a page before interacting. Pass tabId to inspect a specific tab; omit it to use this agent session's current tab. Compact accessibility detail is the default and returns relevant semantic nodes; use mode='full' only for raw accessibility diagnostics. Returns bounded page state, locators, recent diagnostics, and action history. To receive a PNG screenshot or screenshot artifact, you MUST set includeScreenshot=true; omitting it returns semantic data only. Screenshot capture is supported for background-only tabs opened with open=false. Oversized pages are reduced to stay inside the byte budget: check the returned truncated array for the field names that were shortened before trusting them as complete.",
