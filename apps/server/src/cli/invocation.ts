...
import { HostProcessArguments } from "@shuv2code/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `shuv2code` command.
 */
...
/**
 * The `shuv2code` package spec to suggest. The literal spec the user typed (e.g.
 * `shuv2code@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "shuv2code@nightly" : "shuv2code";
}

/**
 * Render a `shuv2code <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx shuv2code connect` suggests
 * `npx shuv2code serve`, a global install suggests `shuv2code serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `shuv2code ${input.subcommand}`;
  }
