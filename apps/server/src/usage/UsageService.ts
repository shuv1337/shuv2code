 ...
 *
 * The scan reads the provider CLIs' own session files rather than shuv2code's
 * orchestration projections, so usage covers turns driven outside shuv2code too.
...
export const make = Effect.gen(function* () {
  ...
  const readFileRecords = (
    ...
    Effect.gen(function* () {
      ...
      const parsed = yield* Effect.promise(() => readTranscriptRecords(filePath));
