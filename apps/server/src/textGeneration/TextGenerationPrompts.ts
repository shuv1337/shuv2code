...
import type { ChatAttachment } from "@shuv2code/contracts";
...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this shuv2code thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing shuv2code thread so the user can recognize it weeks later.
