...
import { isHostWindows } from "@shuv2code/shared/hostProcess";
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "}",
        "",
        "const argsMustContain = process.env.SHUV2CODE_FAKE_CLAUDE_ARGS_MUST_CONTAIN;",
        "if (argsMustContain && !args.includes(argsMustContain)) {",
        '  fail("args missing expected content", 2);',
        "}",
        "",
        "const argsMustNotContain = process.env.SHUV2CODE_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;",
        "if (argsMustNotContain && args.includes(argsMustNotContain)) {",
        '  fail("args contained forbidden content", 3);',
        "}",
        "",
        "const stdinMustContain = process.env.SHUV2CODE_FAKE_CLAUDE_STDIN_MUST_CONTAIN;",
        "if (stdinMustContain && !stdinContent.includes(stdinMustContain)) {",
        '  fail("stdin missing expected content", 4);',
        "}",
        "",
        "const configDirMustBe = process.env.SHUV2CODE_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;",
        ...
        "}",
        "",
        "const stderrText = process.env.SHUV2CODE_FAKE_CLAUDE_STDERR;",
        "if (stderrText) {",
        '  process.stderr.write(stderrText + "\\n");',
        "}",
        "",
        'process.stdout.write(process.env.SHUV2CODE_FAKE_CLAUDE_OUTPUT ?? "");',
        "process.exitCode = Number(process.env.SHUV2CODE_FAKE_CLAUDE_EXIT_CODE ?? 0);",
        "",
      ].join("\n"),
    );
