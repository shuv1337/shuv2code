...
const makeTestContext = Effect.fn("test.makeTestContext")(function* () {
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "shuv2code-boot-service-test-" });
  ...
  return {
    ...
    dirs: {
      home: root,
      baseDir: path.join(root, ".shuv2code"),
      logsDir: path.join(root, ".shuv2code", "userdata", "logs"),
      stableEntry,
    },
  };
});
...
it("renders a systemd unit with absolute paths and append-mode logging", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    shuv2codeEntryPath:
      "/home/theo/.shuv2code/runtime/versions/0.0.27/node_modules/shuv2code/dist/bin.mjs",
  ...
  assert.equal(
    ...
    [
      ...
      "Type=simple",
      "WorkingDirectory=%h",
      "Environment=SHUV2CODE_HOME=/home/theo/.shuv2code",
      "Environment=SHUV2CODE_BOOT_SERVICE_UNIT=shuv2code.service",
      "ExecStart=/usr/local/bin/node /home/theo/.shuv2code/runtime/versions/0.0.27/node_modules/shuv2code/dist/bin.mjs serve",
      "Restart=always",
      "RestartSec=5",
      "StandardOutput=append:/home/theo/.shuv2code/userdata/logs/boot-service.log",
      "StandardError=append:/home/theo/.shuv2code/userdata/logs/boot-service.log",
      "",
      "[Install]",
...
it("quotes systemd values containing spaces and escapes percent specifiers", () => {
  ...
  assert.equal(
    BootService.quoteSystemdValue("/home/me/shuv2code Data"),
    '"/home/me/shuv2code Data"',
  );
  assert.equal(BootService.quoteSystemdValue("/opt/100%cpu"), "/opt/100%%cpu");

  const unit = BootService.renderBootServiceUnit({
    nodePath: "/home/me/my tools/node",
    shuv2codeEntryPath: "/home/me/shuv2code Data/bin.mjs",
    baseDir: "/home/me/shuv2code Data",
    logPath: "/home/me/100%logs/boot.log",
    unitPath: "/home/me/.config/systemd/user/shuv2code.service",
  });
  assert.include(
    unit,
    'ExecStart="/home/me/my tools/node" "/home/me/shuv2code Data/bin.mjs" serve',
  );
  assert.include(unit, 'Environment=SHUV2CODE_HOME="/home/me/shuv2code Data"');
...
it("flags package-manager cache entry points as ephemeral", () => {
  assert.isTrue(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.npm/_npx/abc123/node_modules/shuv2code/dist/bin.mjs",
    ),
  );
  assert.isTrue(
  ...
  assert.isTrue(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.cache/pnpm/dlx/abc/node_modules/shuv2code/dist/bin.mjs",
    ),
  );
  assert.isTrue(
    BootService.isEphemeralCacheEntry(
      "/home/theo/.bun/install/cache/shuv2code@0.0.27/dist/bin.mjs",
    ),
  );
  assert.isFalse(
    BootService.isEphemeralCacheEntry("/usr/local/lib/node_modules/shuv2code/dist/bin.mjs"),
  );
  assert.isFalse(
    BootService.isEphemeralCacheEntry(
      "/home/theo/dev/pnpm/dlx-tools/shuv2code/node_modules/shuv2code/dist/bin.mjs",
    ),
  );
  assert.isFalse(
    BootService.isEphemeralCacheEntry(
...
it.layer(NodeServices.layer)("BootService", (it) => {
  ...
  it.effect("installs the unit, enables the service, and enables linger", () =>
    Effect.gen(function* () {
      ...
      assert.equal(plan.shuv2codeEntryPath, dirs.stableEntry);
      assert.deepEqual(
        commands.map((entry) => [entry.command, ...entry.args].join(" ")),
        [
          "systemctl --user daemon-reload",
          "systemctl --user enable shuv2code.service",
      ...
      const unitPath = path.join(dirs.home, ".config", "systemd", "user", "shuv2code.service");
      const unit = yield* fs.readFileString(unitPath);
      assert.include(unit, `ExecStart=/usr/local/bin/node ${dirs.stableEntry} serve`);
      assert.include(unit, `Environment=SHUV2CODE_HOME=${dirs.baseDir}`);
  ...
  it.effect("pins a runtime via npm install when running from the npx cache", () =>
    Effect.gen(function* () {
      ...
      const service = yield* BootService.make({
        ...
        logsDir: dirs.logsDir,
        cliVersion: "0.0.27",
        host: makeHost("/home/theo/.npm/_npx/abc/node_modules/shuv2code/dist/bin.mjs"),
      }).pipe(Effect.provide(makeRecordingRunnerLayer(commands)), provideHostRefs(dirs.home));
      ...
      assert.equal(
        plan.shuv2codeEntryPath,
        path.join(runtimeDir, "node_modules", "shuv2code", "dist", "bin.mjs"),
      );
      assert.deepEqual(commands[0], {
        command: "npm",
        args: ["install", "--prefix", runtimeDir, "--no-fund", "--no-audit", "shuv2code@0.0.27"],
      });
  ...
  it.effect("reinstalls a pinned runtime when its entry point is missing", () =>
    Effect.gen(function* () {
      ...
      const plan = yield* service.install;
      yield* fs.makeDirectory(path.dirname(plan.shuv2codeEntryPath), { recursive: true });
      yield* fs.writeFileString(plan.shuv2codeEntryPath, "#!/usr/bin/env node\n");
      yield* fs.remove(plan.shuv2codeEntryPath);
      commands.length = 0;
  ...
  it.effect("reports an installed-but-stale unit so the lifecycle can offer a repair", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(
        path.join(unitDir, "shuv2code.service"),
        "[Service]\nExecStart=/old/node /old/shuv2code serve\n",
      );
  ...
  it.effect("fails on non-Linux platforms without touching the filesystem", () =>
    Effect.gen(function* () {
      ...
      const service = yield* BootService.make({
        ...
        logsDir: dirs.logsDir,
        cliVersion: "0.0.27",
        host: makeHost("/usr/local/lib/node_modules/shuv2code/dist/bin.mjs"),
      }).pipe(
        Effect.provide(makeRecordingRunnerLayer(commands)),
      ...
      assert.isFalse(
        yield* fs.exists(path.join(dirs.home, ".config", "systemd", "user", "shuv2code.service")),
      );
  ...
  it.effect("removes the unit file when an activation step fails", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        commands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user disable --now shuv2code.service",
        ),
      );
    }),
  );
  ...
  it.effect("restores the previous unit when a repair cannot activate", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(
        repairCommands.some(
          ({ command, args }) =>
            command === "systemctl" && args.join(" ") === "--user restart shuv2code.service",
        ),
      );
    }),
  );
