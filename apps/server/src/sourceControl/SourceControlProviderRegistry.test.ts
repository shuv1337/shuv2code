...
import { VcsRepositoryDetectionError } from "@shuv2code/contracts";
...
it.effect("routes GitHub remotes to the GitHub provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@github.com:shuv1337/shuv2code.git" }],
    });
...
it.effect("routes Bitbucket remotes to the Bitbucket provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@bitbucket.org:shuv1337/shuv2code.git" }],
    });
