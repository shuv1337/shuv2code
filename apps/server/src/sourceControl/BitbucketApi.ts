...
import { sanitizeBranchFragment } from "@shuv2code/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@shuv2code/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  ...
  accessToken: Config.string("SHUV2CODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("SHUV2CODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("SHUV2CODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
export class BitbucketUnsupportedHostError extends Schema.TaggedErrorClass<BitbucketUnsupportedHostError>()(
  ...
  get detail(): string {
    return `Bitbucket host ${this.hostname} is not supported.`;
  }
...
function checkoutBranchName(input: {
  ...
  return `shuv2code/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
