...
describe("theme files", () => {
  it("derives a readable palette from extreme simple-editor colors", () => {
    ...
    // Status colors fall back to shuv2code's standard red and amber rather than
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...PLUM_CHAT_THEME,
      colors: { ...PLUM_CHAT_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: PLUM_CHAT_THEME.id,
      name: PLUM_CHAT_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(PLUM_CHAT_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(PLUM_CHAT_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(PLUM_CHAT_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(PLUM_CHAT_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(PLUM_CHAT_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(PLUM_CHAT_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Plum Chat palette faithful and readable", () => {
    expectThemeColors(PLUM_CHAT_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(PLUM_CHAT_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(PLUM_CHAT_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [PLUM_CHAT_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy plum-chat-dark preference to dark Plum Chat", () => {
    expect(getThemeDefinition("plum-chat-dark")).toBe(PLUM_CHAT_THEME);
    expect(getThemePreferenceMode("plum-chat-dark")).toBe("dark");
    expect(resolveThemeAppearance("plum-chat-dark", true, false)).toBe("dark");
    expect(resolveDesktopTheme("plum-chat-dark", false)).toBe("dark");
    expect(isKnownThemePreference("plum-chat-dark")).toBe(true);
  });

  it("resolves legacy prefixed ids onto the renamed themes", () => {
    for (const [legacy, theme] of [
      ["legacy-grove", GROVE_THEME],
      ["legacy-ocean", OCEAN_THEME],
      ["legacy-ember", EMBER_THEME],
      ["legacy-iris", IRIS_THEME],
    ] as const) {
    ...
    expect(canonicalThemePreference("plum-chat-dark")).toBe("plum-chat-dark");
    // A stored mix that predates the rename resolves to the new ids.
    expect(
      parseThemeHalves(JSON.stringify({ light: "legacy-ocean", dark: "legacy-grove" })),
    ).toEqual({
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", PLUM_CHAT_THEME.id, GROVE_THEME.id]) {
