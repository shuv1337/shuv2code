...
export const PLUM_CHAT_THEME_ID = "plum-chat" as const;
export const PLUM_CHAT_THEME_LABEL = "Plum Chat";
...
export const CUSTOM_THEMES_STORAGE_KEY = "shuv2code:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "shuv2code:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "shuv2code:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "shuv2code:theme-halves:v1";

const LEGACY_PLUM_CHAT_DARK_THEME_ID = "plum-chat-dark";
...
const RESERVED_THEME_IDS = new Set([
  ...
  EMBER_THEME_ID,
  IRIS_THEME_ID,
  LEGACY_PLUM_CHAT_DARK_THEME_ID,
  "legacy-grove",
  "legacy-ocean",
  "legacy-ember",
  "legacy-iris",
...
// Earlier builds shipped every maintainer theme under a legacy prefix; only
// the Plum Chat palette keeps a variant id. Stored preferences and mixes with
// the old ids stay readable through this alias table.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_PLUM_CHAT_DARK_THEME_ID]: PLUM_CHAT_THEME_ID,
  "legacy-grove": GROVE_THEME_ID,
  "legacy-ocean": OCEAN_THEME_ID,
  "legacy-ember": EMBER_THEME_ID,
  "legacy-iris": IRIS_THEME_ID,
};
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_PLUM_CHAT_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark Plum Chat palette as a separate theme. Keep
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_PLUM_CHAT_DARK_THEME_ID ? "dark" : null;
}
 ...
 */
// Measured from the live the upstream chat app default theme. Translucent chat surfaces are
// flattened over --chat-background so this opaque palette reproduces the
// pixels users see after Plum Chat's blur and noise layers are composited.
// Foreground pairs deviate where necessary to keep normal text at WCAG AA.
const PLUM_CHAT_LIGHT_COLORS: ThemeColors = {
  canvas: "#fdf7fd",
  // shuv2code's workspace header belongs to the chat panel, so keep it seamless
  // with the light chat canvas rather than mapping it to Plum Chat's outer shell.
  chrome: "#fdf7fd",
  toolbar: "#fdf7fd",
  toolbarForeground: "#501854",
  toolbarBorder: "#efbdeb",
  // Plum Chat's light chrome controls sit on its pale gradient-noise surface,
  ...
  // Plum Chat uses a light lavender code surface in light mode. Keeping the
  // dark plum pair here also leaked the dark palette into shuv2code's diffs.
  codeBackground: "#f5ecf9",
  codeForeground: "#673c8b",
  // The live sidebar is transparent over Plum Chat's outer shell. Use that
...
const PLUM_CHAT_DARK_COLORS: ThemeColors = {
  canvas: "#1f1a24",
  // shuv2code's workspace header belongs to the chat panel, so keep it seamless
  // with the canvas rather than mapping it to Plum Chat's outer shell.
  chrome: "#1f1a24",
  toolbar: "#1f1a24",
  ...
  // Cards and panels stay in Plum Chat's plum surface family. Near-black here
  ...
  // Diffs and file previews are full workspace surfaces in shuv2code. Keep them
  ...
  // Plum Chat draws the chat panel edge in this muted pink. The resize rail uses
...
/**
 * The palette shuv2code wears with no theme installed, captured from the app's
...
/**
 * The standard shuv2code look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Plum Chat palette used to fill roles omitted by theme
 * files.
 */
...
/**
 * The status colors shuv2code shows without a theme, read from the app's own
...
export const PLUM_CHAT_THEME: ThemeDefinition = {
  id: PLUM_CHAT_THEME_ID,
  label: PLUM_CHAT_THEME_LABEL,
  appearance: "light",
  colors: decodeThemeColors(PLUM_CHAT_LIGHT_COLORS),
  variants: {
    dark: decodeThemeColors(PLUM_CHAT_DARK_COLORS),
  },
  sidebarArtwork: true,
};
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? PLUM_CHAT_THEME.variants!.dark! : PLUM_CHAT_THEME.colors;
}

/**
 * A companion action color in the Plum Chat mold. This gives send buttons,
