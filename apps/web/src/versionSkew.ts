import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@shuv2code/contracts";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "shuv2code:version-mismatch-dismissals:v1";
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same shuv2code version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx shuv2code@${targetVersion}`;
}
...
export function serverUpdateGuidance(
  ...
  switch (capability) {
    ...
    case "desktop-managed":
      return `The ${serverLabel} is run by the shuv2code desktop app on its machine — update the desktop app there to sync them.`;
    default:
