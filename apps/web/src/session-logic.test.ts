...
describe("derivePendingUserInputs", () => {
  ...
  it("carries typed custom-only text from a pending session prompt into the submit payload", () => {
    ...
    expect(composerSubmitPath).toContain("submitComposerDraft(");
    expect(composerSubmitPath).toContain('"pending-user-input"');
    expect(composerSubmitPath).toContain("onSend(sendEvent)");
  ...
  it("preserves MCP server, tool, arguments, and results for expanded display", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "mcp-tool-done",
        kind: "tool.completed",
        summary: "shuv2code · preview_status",
        payload: {
          itemType: "mcp_tool_call",
          title: "shuv2code · preview_status",
          data: { item },
        },
      }),
    ];
    ...
    const [entry] = deriveWorkLogEntries(activities);
    expect(entry?.toolTitle).toBe("shuv2code · preview_status");
