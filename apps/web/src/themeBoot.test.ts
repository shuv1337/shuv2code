...
const THEME_STORAGE_KEY = "shuv2code:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Plum Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "plum-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Plum Chat",
      storage: {
        [THEME_STORAGE_KEY]: "plum-chat",
    ...
    {
      name: "a legacy legacy-grove preference resolves through the alias",
      storage: { [THEME_STORAGE_KEY]: "legacy-grove", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "legacy plum-chat-dark resolves to dark Plum Chat",
      storage: { [THEME_STORAGE_KEY]: "plum-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("plum-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "shuv2code:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("plum-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(PLUM_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "plum-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "shuv2code:theme-halves:v1": JSON.stringify({ dark: "legacy-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "plum-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "shuv2code:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("plum-chat");
    expect(boot.isDark).toBe(true);
  });
