...
import type { EnvironmentId, VoiceControllerIdentity } from "@shuv2code/contracts";
...
export function VoiceSurface({ environmentId }: { readonly environmentId: EnvironmentId }) {
  ...
  const controllerThreadId = sessionHere
    ...
    : lookup.type === "ready"
      ...
      : null;
  const controllerRef = useMemo(
    () => (controllerThreadId ? scopeThreadRef(environmentId, controllerThreadId) : null),
    [controllerThreadId, environmentId],
  );
  const messages = useThreadMessages(controllerRef);
                  ...
                  className={
                    message.role === "user"
                      ...
                      : message.role === "assistant"
                        ? "max-w-[92%]"
                        : "max-w-[92%] text-muted-foreground"
                  }
                >
