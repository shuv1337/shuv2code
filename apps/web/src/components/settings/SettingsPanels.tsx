...
} from "@shuv2code/shared/backgroundActivitySettings";
...
function ProviderLastChecked({ lastCheckedAt }: { lastCheckedAt: string | null }) {
  useRelativeTimeTick();
  const state = getRelativeTimeState(lastCheckedAt);
  return state.status === "missing" ? null : (
    <span className="text-[11px] text-muted-foreground/60">
      {state.status === "invalid"
        ? "Checked unavailable"
        : `Checked ${state.value}${state.suffix ? ` ${state.suffix}` : ""}`}
    </span>
  );
}
        ...
        <SettingsRow
          ...
          control={
            <Select
              ...
              onValueChange={(value) => {
                if (!value || value === selectedHostedAppChannel) return;
    ...
    <SettingsPageContainer>
      <SettingsSection id="appearance" title="Appearance">
        <SettingsRow
          ...
          description="Choose how shuv2code looks across the app."
          resetAction={
            theme !== "system" ? (
