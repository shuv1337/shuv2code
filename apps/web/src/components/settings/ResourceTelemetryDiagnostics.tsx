...
function categoryLabel(category: ResourceTelemetryProcessCategory): string {
  switch (category) {
    ...
    case "resource-monitor":
      return "Monitor";
    case "unknown-shuv2code":
      return "Shuv2Code process";
  }
}
...
export function ResourceTelemetryDiagnostics() {
  ...
  const allShuv2Code = snapshot?.groups.allShuv2Code;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allShuv2Code ? `${allShuv2Code.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={
                allShuv2Code
                  ? `${formatCpuTime(allShuv2Code.cpuTimeMs)} observed CPU time`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allShuv2Code ? formatBytes(allShuv2Code.currentRssBytes) : "..."}
              detail={
                allShuv2Code
                  ? `${formatBytes(allShuv2Code.peakRssBytes)} combined process peaks`
                  : undefined
              }
            />
            <IconStat
              ...
              label="Process count"
              value={allShuv2Code ? String(allShuv2Code.processCount) : "..."}
              detail={
                allShuv2Code
                  ? `${allShuv2Code.processStarts} starts · ${allShuv2Code.processExits} exits`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allShuv2Code ? formatRate(allShuv2Code.ioReadBytesPerSecond) : "..."}
              detail={
                allShuv2Code ? `${formatBytes(allShuv2Code.ioReadBytes)} observed` : undefined
              }
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allShuv2Code ? formatRate(allShuv2Code.ioWriteBytesPerSecond) : "..."}
              detail={
                allShuv2Code ? `${formatBytes(allShuv2Code.ioWriteBytes)} observed` : undefined
              }
              tone={
                allShuv2Code && allShuv2Code.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allShuv2Code && allShuv2Code.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known Shuv2Code operations so process spikes can be correlated with
