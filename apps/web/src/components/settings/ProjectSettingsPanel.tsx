...
import { scopeProjectRef } from "@shuv2code/client-runtime/environment";
...
import type {
  ...
  Shuv2CodeProjectFileScript,
  ThreadEnvMode,
} from "@shuv2code/contracts";
import { resolveEnvModeLabel } from "../BranchToolbar.logic";
import { createModelSelection } from "@shuv2code/shared/model";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@shuv2code/shared/keybindings";
...
import { useShuv2CodeProjectFileState } from "../../hooks/useShuv2CodeProjectFileScripts";
...
function ProjectDetail({ group }: { group: SidebarProjectSnapshot }) {
  ...
  const projectFile = useShuv2CodeProjectFileState(
  ...
  // repo's shuv2code.json value when present, otherwise the global setting.
  const inheritedEnvMode = projectFile.file?.defaultThreadEnvMode ?? settings.defaultThreadEnvMode;
  const inheritedEnvModeSource =
    projectFile.file?.defaultThreadEnvMode != null ? "shuv2code.json" : "global";
  const importableScripts = useMemo(
    () =>
      projectFile.scripts.filter(
        (fileScript) =>
          !scripts.some(
            (script) =>
  ...
  const importFileScript = useCallback(
    async (fileScript: Shuv2CodeProjectFileScript) => {
    ...
    <>
      <SettingsPageContainer>
        ...
        <SettingsSection title="New threads">
          ...
          <SettingsRow
            title="Workspace"
            description="Where new threads in this project start. Overrides shuv2code.json and the global default; applies to every checkout in this group."
            resetAction={
              storedEnvMode !== null ? (
                <SettingResetButton
              ...
              >
                ...
                <SelectPopup align="end" alignItemWithTrigger={false}>
                  <SelectItem value="inherit">
                    {group.memberProjects.length > 1
                      ? "Default (each checkout's shuv2code.json or global setting)"
        ...
        >
              ...
              {importableScripts.length > 0 ? (
                <Menu>
                  ...
                  <MenuPopup align="end" className="w-72">
                    <MenuGroup>
                      <MenuGroupLabel>Import from shuv2code.json</MenuGroupLabel>
          ...
          {projectFile.status === "invalid" ? (
            <SettingsRow
              title="shuv2code.json is invalid"
              description="A shuv2code.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
