import type { EnvironmentId, ProjectId, PullRequestListEntry } from "@shuv2code/contracts";
...
function entry(
  ...
  return {
    ...
    url: `https://github.com/shuv1337/shuv2code/pull/${overrides.number}`,
...
describe("the list snapshot across a reload", () => {
  ...
  const data = {
    ...
    truncated: true,
    nextCursors: { "shuv1337/shuv2code": "cursor-1" },
  } as never;
  ...
  it("rejects a snapshot whose rows do not decode as entries", () => {
    const storage = makeStorage();
    storage.setItem(
      "shuv2code.pullRequests.list:env-1",
  ...
  it("shrugs off corrupt storage and no storage at all", () => {
    const storage = makeStorage();
    storage.setItem("shuv2code.pullRequests.list:env-1", "{not json");
    expect(readPullRequestListSnapshot(storage, "env-1")).toBeNull();
...
describe("merging the environments' own listings", () => {
  ...
  it("keeps each environment's continuation to itself", () => {
    const merged = mergePullRequestLists([
      [ENV_1, answer({ nextCursors: { "github.com shuv1337/shuv2code": "cursor-1" } })],
      [ENV_2, answer()],
    ]);
    expect(merged?.nextCursors).toEqual({
      [ENV_1]: { "github.com shuv1337/shuv2code": "cursor-1" },
    });
  });
...
describe("colon-namespaced labels typed as a search", () => {
  ...
  it("leaves a pasted link alone rather than naming a label after its scheme", () => {
    const parsed = parsePullRequestQuery("https://github.com/shuv1337/shuv2code/pull/1");
    expect(parsed.filters.labels).toBeUndefined();
    expect(parsed.text).toBe("https://github.com/shuv1337/shuv2code/pull/1");
  });
