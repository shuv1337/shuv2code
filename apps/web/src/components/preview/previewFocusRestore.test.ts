...
describe("preview focus restoration", () => {
  ...
  it("does not steal focus when the user moved to another target", () => {
    const focus = vi.fn();
    const initialTarget = { isConnected: true, focus };
    const currentTarget = { isConnected: true, focus: vi.fn() };
    const restore = capturePreviewFocusRestorer(initialTarget, () => currentTarget);
