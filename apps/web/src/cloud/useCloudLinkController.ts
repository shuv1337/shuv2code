...
/**
 * Drives the primary environment's shuv2code connect link. shuv2code connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update shuv2code connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[shuv2code-connect] Could not update shuv2code connect", {
      message,
      traceId,
      cause,
    });
    ...
    toastManager.add({
      type: "error",
      title: "Could not update shuv2code connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn shuv2code connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to shuv2code connect before enabling this."));
        return false;
      }
