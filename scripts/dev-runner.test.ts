...
const devServerInput = {
  mode: "dev:server",
  shuv2codeHome: "/tmp/shuv2code-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
    ...
    it.effect("lets Vite+ honor the desktop dev task graph", () =>
      Effect.sync(() => {
        assert.deepStrictEqual(getDevRunnerModeArgs("dev:desktop"), [
          "run",
          "--filter=@shuv2code/desktop",
          "--filter=@shuv2code/web",
          "dev",
        ]);
      }),
    );
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit SHUV2CODE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "SHUV2CODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "SHUV2CODE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHUV2CODE_HOME, undefined);
        assert.equal(env.SHUV2CODE_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHUV2CODE_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("requires the browser flag even when the environment enables auto-open", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { SHUV2CODE_NO_BROWSER: "0" },
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: undefined,
          browser: false,
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: "/tmp/custom-shuv2code",
          browser: false,
        ...
        assert.equal(env.SHUV2CODE_HOME, path.resolve("/tmp/custom-shuv2code"));
        assert.equal(env.SHUV2CODE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.SHUV2CODE_NO_BROWSER, "1");
        assert.equal(env.SHUV2CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.SHUV2CODE_LOG_WS_EVENTS, "1");
        assert.equal(env.SHUV2CODE_HOST, "0.0.0.0");
      ...
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            ...
            T3_BOOT_SERVICE_UNIT: "shuv2code.service",
          },
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: undefined,
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            SHUV2CODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: undefined,
        ...
        assert.equal(env.SHUV2CODE_MODE, "web");
        assert.equal(env.SHUV2CODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHUV2CODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom shuv2codeHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          ...
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: "/tmp/my-shuv2code",
          browser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
          host: undefined,
        ...
        assert.equal(env.SHUV2CODE_HOME, path.resolve("/tmp/my-shuv2code"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHUV2CODE_MODE, undefined);
        assert.equal(env.SHUV2CODE_NO_BROWSER, undefined);
        assert.equal(env.SHUV2CODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.SHUV2CODE_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.SHUV2CODE_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { SHUV2CODE_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.SHUV2CODE_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (SHUV2CODE_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/SHUV2CODE_HOST has to be probed as well.
    ...
    // Only the backend honours --host/SHUV2CODE_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { SHUV2CODE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, [
          "SHUV2CODE_PORT_OFFSET",
          "SHUV2CODE_DEV_INSTANCE",
        ]);
    ...
    // Base-dir precedence (--home-dir > worktree .shuv2code > ambient SHUV2CODE_HOME)
    ...
    it.effect("ignores an ambient SHUV2CODE_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { SHUV2CODE_HOME: "/home/user/.shuv2code" },
          serverOffset: 0,
          webOffset: 0,
          shuv2codeHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit SHUV2CODE_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { SHUV2CODE_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults SHUV2CODE_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.SHUV2CODE_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit SHUV2CODE_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.SHUV2CODE_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves SHUV2CODE_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.SHUV2CODE_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("shuv2code home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "shuv2code-devrunner-"));
      ...
      const spawnedHome = (input: {
        readonly shuv2codeHome: string | undefined;
        readonly cwd: string;
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({
            ...devServerInput,
            shuv2codeHome: input.shuv2codeHome,
          }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            ...
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { SHUV2CODE_HOME: input.ambientHome },
            ),
          );

          return captured?.SHUV2CODE_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            shuv2codeHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.shuv2code",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".shuv2code"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .shuv2code over an ambient SHUV2CODE_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient SHUV2CODE_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            shuv2codeHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.shuv2code"));
        }),
      );
