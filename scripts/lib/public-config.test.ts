...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.SHUV2CODE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.SHUV2CODE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.SHUV2CODE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.SHUV2CODE_RELAY_URL).toBeUndefined();
    expect(env.VITE_SHUV2CODE_RELAY_URL).toBeUndefined();
    expect(env.SHUV2CODE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.SHUV2CODE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.SHUV2CODE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "SHUV2CODE_CLERK_PUBLISHABLE_KEY=pk_root\nSHUV2CODE_CLERK_JWT_TEMPLATE=template_root\nSHUV2CODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nSHUV2CODE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "SHUV2CODE_CLERK_PUBLISHABLE_KEY=pk_local\nSHUV2CODE_CLERK_JWT_TEMPLATE=template_local\nSHUV2CODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nSHUV2CODE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).SHUV2CODE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          SHUV2CODE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          SHUV2CODE_CLERK_JWT_TEMPLATE: "template_ci",
          SHUV2CODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          SHUV2CODE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      SHUV2CODE_RELAY_URL: "https://ci.example.test",
      VITE_SHUV2CODE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        SHUV2CODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_SHUV2CODE_RELAY_URL: "https://legacy.example.test",
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
    ...
    expect(
      loadRepoEnv({
        baseEnv: {
          ...
          SHUV2CODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          SHUV2CODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      SHUV2CODE_RELAY_URL: "https://relay.example.test",
      VITE_SHUV2CODE_RELAY_URL: "https://relay.example.test",
      SHUV2CODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      SHUV2CODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      SHUV2CODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "shuv2code-public-config-"));
