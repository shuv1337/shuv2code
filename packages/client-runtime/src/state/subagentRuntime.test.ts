...
import { classifyTaskAgentKind, type OrchestrationThreadActivity } from "@shuv2code/contracts";
import {
...
describe("terminal robustness", () => {
  ...
  it("a late start after a terminal task.updated does not reopen the run", () => {
    const agents = fold([
      activity("task.updated", { taskId: "task-1", status: "failed", role: "worker" }),
      activity("task.started", { taskId: "task-1", taskType: "local_agent", title: "Late" }),
    ]);
    expect(agents).toHaveLength(1);
  ...
  it("duplicate completions keep the FIRST result, not the last", () => {
    const agents = fold([
      activity("task.started", { taskId: "task-2", taskType: "local_agent" }),
      activity("task.completed", {
        taskId: "task-2",
        status: "completed",
