...
describe("ssh tunnel scripts", () => {
  it("builds the remote shuv2code runner with npx and npm fallbacks", () => {
    const script = buildRemoteT3RunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "SHUV2CODE_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec shuv2code "$@"');
    assert.include(script, "exec npx --yes 'shuv2code@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'shuv2code@latest' -- \"$@\"");
    assert.include(script, "could not install 'shuv2code@latest'");
    assert.include(
      script,
      "require_installed_shuv2code_cli npx --yes --package 'shuv2code@latest'",
    );
    assert.include(
      script,
      "require_installed_shuv2code_cli npm exec --yes --package 'shuv2code@latest'",
    );
    assert.include(script, "npm produced no shuv2code executable");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `SHUV2CODE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for SHUV2CODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
  ...
  it("does not hard-code a remote node engine range", () => {
    ...
    assert.include(script, "SHUV2CODE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote shuv2code runner", () => {
    const script = buildRemoteT3RunnerScript({
      packageSpec: "shuv2code@nightly; touch /tmp/shuv2code-owned",
    });

    assert.include(script, "exec npx --yes 'shuv2code@nightly; touch /tmp/shuv2code-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'shuv2code@nightly; touch /tmp/shuv2code-owned' -- \"$@\"",
    );
    assert.include(
      script,
      "require_installed_shuv2code_cli npx --yes --package 'shuv2code@nightly; touch /tmp/shuv2code-owned'",
    );
    assert.notInclude(script, "exec npx --yes shuv2code@nightly; touch /tmp/shuv2code-owned");
  });

  it("builds the remote shuv2code runner with a node script override", () => {
    const script = buildRemoteT3RunnerScript({
    ...
    assert.include(
      script,
      "SHUV2CODE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$SHUV2CODE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote shuv2code runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `SHUV2CODE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote shuv2code server did not become ready");
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "shuv2code@nightly" }),
      "shuv2code@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "shuv2code@nightly" }),
      "shuv2code@nightly",
    );
    assert.include(
