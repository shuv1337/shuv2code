...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@shuv2code/contracts";
...
const PUBLISHABLE_SHUV2CODE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteT3CliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_SHUV2CODE_VERSION_PATTERN.test(appVersion)) {
    return `shuv2code@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "shuv2code@nightly";
  }

  return input.updateChannel === "nightly" ? "shuv2code@nightly" : "shuv2code@latest";
}
