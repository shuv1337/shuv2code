...
} from "@shuv2code/shared/httpReadiness";
import * as NetService from "@shuv2code/shared/Net";
import { extractJsonObject, fromLenientJson } from "@shuv2code/shared/schemaJson";
import { satisfiesSemverRange } from "@shuv2code/shared/semver";
...
remote_node_satisfies_engine() {
  SHUV2CODE_NODE_ENGINE_RANGE=@@SHUV2CODE_NODE_ENGINE_RANGE@@
  if [ -z "$SHUV2CODE_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$SHUV2CODE_NODE_ENGINE_RANGE" <<'NODE'
@@SHUV2CODE_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for SHUV2CODE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$SHUV2CODE_NODE_BIN/node" ]; then
        PATH="$SHUV2CODE_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@SHUV2CODE_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
SHUV2CODE_NODE_SCRIPT_PATH=@@SHUV2CODE_NODE_SCRIPT_PATH@@
if [ -n "$SHUV2CODE_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$SHUV2CODE_NODE_SCRIPT_PATH" "$@"
fi
if command -v shuv2code >/dev/null 2>&1; then
  exec shuv2code "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (shuv2code depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a shuv2code executable. \`npx --yes\` then exits 0 without
...
require_installed_shuv2code_cli() {
  SHUV2CODE_CLI_PATH="$("$@" -- sh -c 'command -v shuv2code' || true)"
  if [ -n "$SHUV2CODE_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no shuv2code executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@SHUV2CODE_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_shuv2code_cli npx --yes --package @@SHUV2CODE_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@SHUV2CODE_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_shuv2code_cli npm exec --yes --package @@SHUV2CODE_PACKAGE_SPEC@@ || exit 1
  exec npm exec --yes @@SHUV2CODE_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the shuv2code CLI and could not install @@SHUV2CODE_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@SHUV2CODE_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.shuv2code/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.shuv2code"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-shuv2code.sh"
RUNNER_NEXT="$STATE_DIR/run-shuv2code.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@SHUV2CODE_DEFAULT_REMOTE_PORT@@" "@@SHUV2CODE_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@SHUV2CODE_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@SHUV2CODE_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@SHUV2CODE_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@SHUV2CODE_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@SHUV2CODE_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@SHUV2CODE_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env SHUV2CODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@SHUV2CODE_READY_TIMEOUT_MS@@"; then
    printf 'Remote shuv2code server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.shuv2code/ssh-launch/@@SHUV2CODE_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.shuv2code"
RUNNER_FILE="$STATE_DIR/run-shuv2code.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "shuv2code@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      SHUV2CODE_PACKAGE_SPEC: packageSpec,
      SHUV2CODE_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      SHUV2CODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}
...
export function buildRemoteNodeEnvScript(input?: RemoteT3RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      SHUV2CODE_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      SHUV2CODE_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    SHUV2CODE_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    SHUV2CODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
    SHUV2CODE_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    SHUV2CODE_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    SHUV2CODE_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    SHUV2CODE_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    SHUV2CODE_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    SHUV2CODE_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    SHUV2CODE_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}
...
export function buildRemotePairingScript(
  ...
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    SHUV2CODE_STATE_KEY: remoteStateKey(target),
    SHUV2CODE_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteT3RunnerScript(input)),
  });
}
...
>()("@shuv2code/ssh/tunnel/SshEnvironmentManager") {
