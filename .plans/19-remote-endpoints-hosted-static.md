...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later shuv2code Tunnel
- users can generate a hosted pairing link for `app.shuv2code.example`
...
This plan intentionally leaves the paid shuv2code cloud tunnel fabric out of scope. It defines the OSS foundation that shuv2code Tunnel should later plug into.
...
- `https://app.shuv2code.example` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.shuv2code.example` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.shuv2code.example/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.shuv2code.example`.
...
- `app.shuv2code.example` works as a real client shell.
...
- `app.shuv2code.example` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.shuv2code.example`.
...
## Phase 4: Future shuv2code Tunnel Provider
...
- create endpoint with `provider: "shuv2code-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of shuv2code should only see an `AdvertisedEndpoint`.
