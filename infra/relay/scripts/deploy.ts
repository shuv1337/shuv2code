...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    SHUV2CODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    SHUV2CODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    SHUV2CODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("SHUV2CODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("SHUV2CODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "Shuv2CodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the shuv2code relay through Alchemy."));
