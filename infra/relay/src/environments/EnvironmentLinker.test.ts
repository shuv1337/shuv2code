...
import { RELAY_LINK_PROOF_TYP } from "@shuv2code/shared/relayJwt";
...
const makeRequest = Effect.gen(function* () {
  ...
  const payload = {
    iss: "shuv2code-env:env-link-test",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(EnvironmentCredentials.EnvironmentCredentials, {
          create: () => Effect.succeed("shuv2code_env_credential_secret"),
...
describe("EnvironmentLinker", () => {
  it.effect("uses verified JWT claims when linking an environment", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.environmentCredential).toBe("shuv2code_env_credential_secret");
